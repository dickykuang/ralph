---
name: ralph-reset
description: Reset Ralph state in .ralph with explicit confirmation and safe failure handling.
---
...
Then use the direct user prompt to confirm:
```
...
You can now run ralph skill to start a new task.
```
...
- After reset, the project is ready for a fresh `ralph` skill invocation
