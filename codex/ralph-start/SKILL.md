...
Run the ralph skill again, review the plain-English walkthrough and diagrams, then reply `ack`.
```
...
5. Suggest `ralph-retro` skill only for deeper walkthrough, questions, or follow-up changes
...
- **Developer Task Fields**: Include `why_this_task_exists`, `plain_english_summary`, `expected_user_visible_change`, and `verification_plan`. If an older task file does not have these fields, synthesize short layman-language replacements before prompting the worker.
...
Next: Run ralph-retro skill only if you want a deeper walkthrough, have questions, or want follow-up changes.
```
