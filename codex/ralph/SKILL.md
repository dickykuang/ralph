...
1. **Display summary**:
   ...
   3. Run start a new Codex session to reset context
   4. Run ralph-start skill to begin execution
   ```
...
[Planning] → Generate PRD and task files
    ↓
[planning_complete] → STOP, remind user the walkthrough was acknowledged, then run start a new Codex session and ralph-start skill
```
...
[planning] → (generate PRD and task files)
    ↓
[planning_complete] → (acknowledged plan ready; user runs start a new Codex session, then ralph-start skill)
    ↓
...
| `verification_plan` | array | Yes | Checks/tests the worker should run or explain if unavailable |
